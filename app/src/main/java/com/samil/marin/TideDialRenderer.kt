...
object TideDialRenderer {
    ...
    private fun drawLabels(canvas: Canvas, cx: Float, cy: Float, size: Int, state: TideState) {
        ...
        val highLabel = "${highEvent.dateTime.format(timeFmt)}h  ${"%.1f".format(highEvent.heightMeters)}m"
        val lowLabel = "${lowEvent.dateTime.format(timeFmt)}h  ${"%.1f".format(lowEvent.heightMeters)}m"

        titlePaint.color = blue
        canvas.drawText("pleamar", cx, size * 0.055f, titlePaint)
        timePaint.color = blue
        canvas.drawText(highLabel, cx, size * 0.115f, timePaint)

        titlePaint.color = red
        canvas.drawText("bajamar", cx, size * 0.900f, titlePaint)
        timePaint.color = red
        canvas.drawText(lowLabel, cx, size * 0.960f, timePaint)
