...
private const val ID_PORTO_MARIN = 15
...
private const val ASSET_FILE = "mareas_marin.json"
...
class TideRepository(private val context: Context) {
    ...
    fun syncFromNetwork(): Boolean {
        ...
        return try {
            ...
            val url = "$BASE_URL?idPorto=$ID_PORTO_MARIN" +
            ...
            val connection = (URL(url).openConnection() as HttpURLConnection).apply {
                ...
                readTimeout = 15000
                setRequestProperty("User-Agent", "Mozilla/5.0 (Android) MareasMarinApp/1.0")
    ...
    /**
     * El asset viene como tripletas [fechaHoraISO, esPleamar(1/0), alturaMetros].
     */
        ...
        return try {
            context.assets.open(ASSET_FILE).bufferedReader().use { reader ->
                ...
                for (i in 0 until arr.length()) {
                    ...
                    val dt = LocalDateTime.parse(triple.getString(0))
                    val isHigh = triple.getInt(1) == 1
                    val altura = if (triple.length() > 2) triple.getDouble(2) else 0.0
